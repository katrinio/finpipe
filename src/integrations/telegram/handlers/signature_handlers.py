...
class SignatureHandlers:
    ...
    def upload_signature(self, telegram_id: int) -> None:
        ...
        self.telegram.send_message(telegram_id, SignatureMessages.Upload.REQUIREMENTS, reply_markup=build_signature_menu())
    ...
    def handle_signature_upload(self, telegram_id: int, file_name: str, file_size: int, file_bytes: bytes) -> None:
        ...
        try:
            ...
            self.telegram.send_message(telegram_id, SignatureMessages.Validation.NOT_PNG, reply_markup=build_signature_menu())
            return
        except SignatureTooLargeError:
            LOGGER.warning("Signature rejected by size for Telegram user %s", telegram_id)
            self.telegram.send_message(telegram_id, SignatureMessages.Validation.TOO_LARGE, reply_markup=build_signature_menu())
            return
        except InvalidSignatureImageError:
            LOGGER.warning("Signature rejected by content for Telegram user %s", telegram_id)
            self.telegram.send_message(telegram_id, SignatureMessages.Validation.UPLOAD_ERROR, reply_markup=build_signature_menu())
            return

        self.state_service.clear_state(telegram_id)
        LOGGER.info("Signature uploaded for Telegram user %s", telegram_id)
        self.telegram.send_message(telegram_id, SignatureMessages.Upload.UPDATED, reply_markup=build_signature_menu())
    ...
    def delete_signature(self, telegram_id: int) -> None:
        ...
        if signature is None:
            ...
            self.telegram.send_message(telegram_id, SignatureMessages.Status.NOT_FOUND, reply_markup=build_signature_menu())
            return

        Signature.delete(telegram_id)
        LOGGER.info("Signature deleted for Telegram user %s", telegram_id)
        self.telegram.send_message(telegram_id, SignatureMessages.Delete.DELETED, reply_markup=build_signature_menu())
    ...
    def signature_status(self, telegram_id: int) -> None:
        ...
        self.telegram.send_message(telegram_id, SignatureMessages.Status.FOUND, reply_markup=build_signature_menu())
