...
class EventAnalytics:
    ...
    @classmethod
    def get_error_counts_by_category(cls) -> dict[str, int]:
        category = func.coalesce(cast(AppEvent.details, JSONB)["category"].astext, "uncategorized")
        with AppEvent.session() as session:
            statement = (
            ...
            return {category_value: count for category_value, count in session.execute(statement).all()}
