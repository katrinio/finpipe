...
def render_transfer_pdf(rendered_docx_path: Path, output_path: Path, data: dict[str, str]) -> None:
    try:
        render_transfer_pdf_with_pages(
            rendered_docx_path=rendered_docx_path,
        ...
        render_transfer_pdf_fallback(
            output_path=output_path,
            data=data,
        )


def render_transfer_pdf_with_pages(rendered_docx_path: Path, output_path: Path) -> None:
    ...
    LOGGER.info("Exporting transfer request PDF with Pages: %s", output_path)
    subprocess.run(
...
def build_pages_export_script(rendered_docx_path: Path, output_path: Path) -> list[str]:
    return [
...
def render_transfer_pdf_fallback(output_path: Path, data: dict[str, str]) -> None:
    ...
    pdf.drawString(
        40,
        y_position - 20,
        "Generated automatically from transfer request template.",
    )
    pdf.save()
