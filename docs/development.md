...
|---|---|
| **Reply keyboard** | Постоянная навигация: главное меню, разделы, возврат домой |
| **Inline keyboard** | Prompt-экраны с подтверждением: отправить инвойс / ответить банку |

**Inline-кнопки** прикреплены к конкретному сообщению и остаются видны после нажатия. Нажатие генерирует `callback_query` с `callback_data` равным тексту кнопки — тот же роутер `_command_handlers` обрабатывает их наравне с обычными командами. После обработки бот автоматически вызывает `answerCallbackQuery`, чтобы убрать индикатор загрузки.

**Reply-keyboard** не используют `one_time_keyboard` — меню всегда доступно.
