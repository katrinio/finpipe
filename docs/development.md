...
Вся навигация построена на **inline-клавиатурах**. Reply-keyboard не используется.
...
├── 📄 Документы
│   ├── 🧾 Инвойс
│   │   ├── 💰 Указать сумму инвойса
│   │   ├── 💶 Текущая сумма инвойса
│   │   ├── 📄 Сгенерировать инвойс
│   │   │   └── [prompt] 📤 Отправить компании | Не отправлять
│   │   └── ◀️ Назад
│   ├── 🏦 Банковский день
│   │   └── [prompt] 📤 Ответить банку | Не отправлять
│   └── ◀️ Назад
├── 👤 Мой профиль
│   ├── 👁 Посмотреть профиль
│   ├── 📥 Шаблон профиля  |  📤 Обновить профиль
│   ├── ✍️ Загрузить подпись  |  🗑 Удалить подпись
│   └── ◀️ Назад
├── 📧 Интеграции
│   ├── 📧 Gmail
│   │   ├── 🔗 Подключить  |  ❌ Отключить
│   │   ├── 📊 Статус  |  🗑 Сбросить историю писем
│   │   └── ◀️ Назад
│   └── ◀️ Назад
└── 📖 Справка
    ├── ❓ Как начать  |  👤 Кто я
...
Нажатие кнопки генерирует `callback_query`. `callback_data` навигационных кнопок имеет префикс `nav:` (например, `nav:documents`, `nav:invoice`). Роутер `_command_handlers` обрабатывает их наравне с текстовыми командами.

После обработки бот вызывает `answerCallbackQuery` (убирает индикатор загрузки) и удаляет сообщение с меню — чтобы чат не засорялся. Удаление происходит только для `nav:`-колбэков; prompt-кнопки (отправить / не отправлять) сообщение не удаляют.
