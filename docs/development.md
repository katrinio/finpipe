...
- ✅/❌ Профиль (компания + реквизиты)
- ✅/❌ Подпись
- ✅/❌ Gmail

После нажатия «▶️ Запустить» — запускается workflow. Если пользователь отказывается отправлять ответ банку (`✖️ Не отправлять`), письмо снимается с отметки обработанного и может быть обработано снова.
