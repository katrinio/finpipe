...
def test_resolve_signature_for_user_prefers_active_user_signature(tmp_path: Path) -> None:
    ...
    SignatureCipher._cipher = Fernet(Fernet.generate_key())
    SignatureCipher.encrypt_file(source, user_signature)
...
def test_resolve_signature_for_user_falls_back_to_default_when_active_signature_file_is_missing(tmp_path: Path) -> None:
    ...
    source.write_bytes(b"signature-bytes")
    SignatureCipher._cipher = Fernet(Fernet.generate_key())
    SignatureCipher.encrypt_file(source, missing_signature)
