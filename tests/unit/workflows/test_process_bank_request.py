...
sys.modules.setdefault(
    ...
    types.SimpleNamespace(fetch_bank_email_workflow=lambda: None),
)
sys.modules.setdefault(
...
def test_main_returns_early_when_no_new_bank_email(monkeypatch) -> None:
    ...
    monkeypatch.setattr(process_bank_request, "fetch_bank_email_workflow", lambda: None)
    ...
    assert result == 0
    assert calls == ["configure_logging", "get_dotenv"]
...
def test_main_generates_all_documents_and_sends_bank_response(monkeypatch) -> None:
    ...
    monkeypatch.setattr(process_bank_request.EnvVar, "get_dotenv", lambda: calls.append("get_dotenv"))
    monkeypatch.setattr(process_bank_request, "TelegramClient", lambda: telegram_client)
    monkeypatch.setattr(process_bank_request, "fetch_bank_email_workflow", lambda: bank_template_path)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda amount: calls.append(("invoice", amount)) or invoice_pdf_path,
    )
