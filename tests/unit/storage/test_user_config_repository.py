...
from src.storage.repositories.user_config_repository import SQLAlchemyUserConfigRepository


def test_user_config_repository_get_by_telegram_id(tmp_path) -> None:
    database = Database(build_sqlite_url(tmp_path / "storage.sqlite3"))
    database.initialize_schema()
    repository = SQLAlchemyUserConfigRepository(database.session)
    ...
    user = repository.get_by_telegram_id(777)

    assert user is not None
    assert user.telegram_id == 777
    assert user.user_name == "alice"
    assert repository.get_by_telegram_id(123) is None
