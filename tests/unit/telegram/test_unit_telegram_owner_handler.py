...
def test_add_user_denies_non_owner(tmp_path: Path) -> None:
    ...
    assert telegram.sent_messages == [CommonMessages.Errors.ACCESS_DENIED]
    assert telegram.sent_messages_with_chat_ids == [(1, CommonMessages.Errors.ACCESS_DENIED)]
    assert telegram.sent_message_payloads == [(1, CommonMessages.Errors.ACCESS_DENIED, build_guest_menu())]
